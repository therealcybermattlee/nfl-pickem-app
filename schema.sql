...
CREATE TABLE IF NOT EXISTS games (
  ...
  status TEXT NOT NULL DEFAULT 'upcoming' CHECK (status IN ('upcoming', 'locked', 'in_progress', 'final')),
  homeScore INTEGER,
  awayScore INTEGER,
  spread REAL,
  overUnder REAL,
...
CREATE TABLE IF NOT EXISTS picks (
  ...
  lockedAt TEXT, -- When the pick was actually submitted/locked
  isLocked BOOLEAN DEFAULT 0, -- Whether the pick can be changed
  autoGenerated BOOLEAN DEFAULT 0, -- If system made the pick automatically
  createdAt TEXT NOT NULL DEFAULT (datetime('now')),
  updatedAt TEXT NOT NULL DEFAULT (datetime('now')),
...
-- Create game_locks table for tracking game lock status
CREATE TABLE IF NOT EXISTS game_locks (
  id TEXT PRIMARY KEY,
  gameId TEXT NOT NULL UNIQUE,
  lockTime TEXT NOT NULL, -- When the game should lock
  isLocked BOOLEAN DEFAULT 0, -- Current lock status
  lastChecked TEXT NOT NULL DEFAULT (datetime('now')), -- Last time status was checked
  autoPicksGenerated BOOLEAN DEFAULT 0, -- Whether auto picks have been generated
  createdAt TEXT NOT NULL DEFAULT (datetime('now')),
  updatedAt TEXT NOT NULL DEFAULT (datetime('now')),
  FOREIGN KEY (gameId) REFERENCES games(id)
);
...
-- Time-lock specific indexes for performance

...
async function handleApiRequest(request: Request, pathname: string, db: D1DatabaseManager, env: Env): Promise<Response> {
  ...
  // Pick History API - Get user's pick history with results
  if (pathname === '/api/picks/history') {
    if (method === 'GET') {
      ...
      try {
        ...
        // Build SQL query with conditional week filter
        let query = `
          SELECT
            p.id as pickId,
            p.gameId,
            p.userId,
            p.teamId as selectedTeamId,
            p.autoGenerated as isAutoGenerated,
            p.createdAt as pickCreatedAt,
            g.id as gameId,
            g.week,
            g.season,
            g.homeTeamId,
          ...
          WHERE p.userId = ? AND g.season = ?
        `

        const params: any[] = [userId, season]
        ...
        query += ' ORDER BY g.week ASC, g.gameDate ASC'

        // Execute query
        const result = await env.DB.prepare(query).bind(...params).all()

        // Format response with isCorrect calculation
        const picks = result.results.map((row: any) => {
          // Calculate isCorrect based on game status
          let isCorrect: boolean | null = null
          if (row.status === 'final' && row.winnerTeamId) {
            isCorrect = row.selectedTeamId === row.winnerTeamId
          }
        ...
        return new Response(JSON.stringify({
          success: false,
          error: error.message || 'Failed to fetch pick history'
        }), {
          status: 500,
          headers: { 'Content-Type': 'application/json', ...corsHeaders }
        })
  ...
  if (pathname === '/api/auth/register') {
    if (method === 'POST') {
      ...
      // Password strength validation (configurable minimum length)
...
async function handleAutoGeneratePicks(db: D1DatabaseManager): Promise<Response> {
  try {
    ...
    for (const game of lockedGames) {
      ...
      // Count existing picks for this game to maintain 50/50 distribution
      const existingPicksResult = await db.query(`
      ...
      const existingPicks = existingPicksResult.results || []
      let homePicks = 0
      let awayPicks = 0

      for (const pick of existingPicks) {
        const pickData = pick as any
        if (pickData.teamId === gameData.homeTeamId) {
          homePicks = pickData.count
        } else if (pickData.teamId === gameData.awayTeamId) {
          awayPicks = pickData.count
        }
      }
      ...
      // Calculate 50/50 distribution for auto-picks
      const totalAutoPicksNeeded = missingPicks.length
      const homePicksNeeded = Math.ceil(totalAutoPicksNeeded / 2)
      const awayPicksNeeded = totalAutoPicksNeeded - homePicksNeeded

      // Create array of team assignments ensuring 50/50 split
      const teamAssignments: string[] = [
        ...Array(homePicksNeeded).fill(gameData.homeTeamId),
        ...Array(awayPicksNeeded).fill(gameData.awayTeamId)
      ]
      ...
      // Generate picks with balanced distribution
      for (let i = 0; i < missingPicks.length; i++) {
        const userData = missingPicks[i] as any
        const assignedTeam = teamAssignments[i]
        ...
        try {
          ...
          `, [pickId, userData.userId, gameData.id, assignedTeam])
